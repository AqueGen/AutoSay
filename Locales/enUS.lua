...
L["Tag navigation note"] = "Greyed rows cannot go out as things stand: everything greys while the addon itself is off, a whole column greys when its channel is off on the General tab or has no trigger switched on above, and single phrases follow their tag - [self] / [newcomers] follow this channel's triggers, role and [morning/evening/night] follow the master switches on the Style tab. Greetings stay live with both join triggers off only while On reconnect is on and the Reconnects list has nothing the addon could say, since that is when a reconnect borrows a greeting."
