...
L["Record session log"] = "Record session log"
L["Record session log desc"] = "Write down what the addon does - every trigger, every message, every refusal and its reason - so a real session can be read back afterwards. Survives a reload, keeps the last 500 lines, and is account-wide. Off by default and free while it is off."
L["Copy log"] = "Copy log"
L["Copy log desc"] = "Open the log in a window you can select and copy out of, for a bug report."
...
L["When someone joins"] = "When someone joins"
L["Goodbyes"] = "Goodbyes"
L["Messages"] = "Messages"
...
L["Send message when timed"] = "Send message when the key is timed"
L["Send message when timed desc"] = "Say something in party chat when the dungeon beats the timer."
L["Send message when depleted"] = "Send message when the timer is missed"
L["Send message when depleted desc"] = "Say something in party chat when the dungeon is finished outside the timer. Off is a fair choice: silence reads better than a cheerful line after a key goes down."
L["On time"] = "On time"
...
L["Reset defaults desc"] = "Put every setting in this profile back the way it shipped, and clear what this character remembers: the hourly budget, who it has already greeted, and which phrase each list said last. Custom messages in this profile are deleted."
L["Reset defaults confirm"] = "Reset this profile to defaults? Your custom messages and every phrase you have ticked or unticked will be lost."
...
L["Nothing selected notice"] = "Nothing is ticked for: %s. The trigger is on, so the addon will try and find nothing to say. Tick a phrase below, or switch the trigger off."
L["Tag navigation note"] = "Greyed rows cannot go out as things stand: everything greys while the addon itself is off, a whole column greys when its channel is off on the General tab or has no trigger switched on above, and a phrase with a role or a [morning/evening/night] tag follows the master switches on the Style tab. On this tab the occasion is the tab itself, so no phrase carries a trigger tag. Greetings for arriving stay live with the join trigger off only while On reconnect is on and the Reconnects list has nothing the addon could say, since that is when a reconnect borrows one."
