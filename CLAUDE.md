...
- The `## Version:` line in `AutoSay.toc` is the **source of truth** — bump it before creating a tag. `version-check.yml` fails the tag push if the tag and the TOC version disagree (a leading `v` is ignored on both sides).
